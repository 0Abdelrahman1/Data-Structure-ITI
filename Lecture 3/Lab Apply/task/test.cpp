...
template<typename T>
T test_insert(const string& name, vector<int> order) {
    T bst;
...
int main()
{
    ...
    test_insert<AVL<int>>("sorted_inc", sorted_inc(N));
    test_insert<AVL<int>>("sorted_dec", sorted_dec(N));
    test_insert<AVL<int>>("alternating", alternating(N));
    test_insert<AVL<int>>("random", random_order(N));

    auto v = random_order(N);
    test_erase<AVL<int>>("erase_sorted", v, sorted_inc(N));
    test_erase<AVL<int>>("erase_reverse", v, sorted_dec(N));
    test_erase<AVL<int>>("erase_random", v, random_order(N));
}